...
#[test]
fn test_prepare_buffer_offset_misalignment_tcp() {
    let _ = env_logger::builder().is_test(true).try_init();
    let listener = TcpListener::bind("127.0.0.1:0").expect("Failed to bind mock server");
    let addr = listener.local_addr().unwrap();
    let port = addr.port();
    ...
    let server_handle = thread::spawn(move || {
        ...
        let mut prepare_payload = Vec::new();
        prepare_payload.write_u32::<LittleEndian>(28).unwrap();
        prepare_payload.push(0xAB); // 5th byte: trailing garbage shifts offset 1 to a huge number
        ...
        // Send CMD_PREPARE_DATA for the chunk
        let mut chunk_prepare_payload = Vec::new();
        chunk_prepare_payload.write_u32::<LittleEndian>(32).unwrap();
        let res = ZKPacket::new(
            CMD_PREPARE_DATA,
            session_id,
            packet.reply_id,
